...
export default function Devices({post:serverPost}){
    const [post,setPost]=useState(serverPost);
    const rout =useRouter();

    useEffect(()=>{
        async function load(){
              const res = await fetch(`http://localhost:4040/allItems/${rout.query.deviceid}`);
   ...
   // console.log(rout);
    return<MainLayout title={post.nameMain}>


        <section className='sec-bc-hd'>
                    <div style={styles.submain}>    
                         <div style={styles.headSerc}>
                           <h1 style={styles.h1}>{post.nameMain}</h1>
                    </div>
            <hr style={styles.hr}></hr>
            <div style={styles.subnewCatalog}>
                    <div style={styles.newCatalog}>
...
Devices.getInitialProps = async ({query,req})=>{
    if(!req){
        return {post:null}
   }
    const res = await fetch(`http://localhost:4040/allItems/${query.deviceid}`);
